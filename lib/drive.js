...
export function favoritesHref(spaceKey) {
  return `${getSpaceConfig(spaceKey).basePath}?favorites=1`;
}
...
export function favoritesFileHref(spaceKey, fileId) {
  return `${favoritesHref(spaceKey)}&file=${fileId}`;
}
