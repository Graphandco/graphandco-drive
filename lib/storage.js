...
export function getStorageConfig(location = "sixmyk") {
  ...
  return {
    location,
    driver: process.env[`STORAGE_${prefix}_DRIVER`] || "s3",
...
/**
 * Clé S3 plate : [prefix/][space]/files/uuid-nom.ext
 * L’arborescence dossiers vit uniquement en base (file_folders).
 */
export function buildObjectKey({ space = "sixmyk", fileName }) {
  const safeName = `${randomUUID()}-${sanitizeFileName(fileName)}`;
  const spaceSegment = sanitizeFolderSegment(space) || "files";
  return withStorageKeyPrefix(`${spaceSegment}/files/${safeName}`);
}
 ...
 * Ex. [{id:1,name:Six-MyK},{id:4,name:Photos}] + root=1 → "Photos"
