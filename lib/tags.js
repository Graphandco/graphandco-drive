...
/** Filtre AND sur plusieurs tags (chaîne CSV ou liste). */
export function tagsAndFilterClause(tags, column = "f.tags") {
  ...
  for (const tag of list) {
    ...
    parts.push(
      `AND CONCAT(',', REPLACE(REPLACE(LOWER(TRIM(${column})), ', ', ','), ';', ','), ',') LIKE ?`
    );
